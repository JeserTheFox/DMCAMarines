...
/obj/vehicle/multitile/root
	...
	var/old_dir
	var/turret_busy = FALSE
	var/turret_dir = NORTH
	var/turret_old_dir
	var/turret_stabilizer = FALSE
...
/obj/vehicle/multitile/root/proc/turret_rotate(var/mob/user, var/direction)
	if(direction == turn(180, turret_dir))
		return

	turret_busy = FALSE
	if(turret_dir in list(EAST, WEST))
		if(direction == SOUTH)
			turret_dir = SOUTH
			return
		...
		if(turret_dir in list(SOUTH, NORTH))
			if(direction == EAST)
				turret_dir = EAST
				return
			else
...
/obj/vehicle/multitile/root/proc/try_rotate(var/deg, var/mob/user, var/force = 0)
	...
	if(!try_move(linked_objs, null, 1))
		...
		revert_locs()
		return 0
	if(!turret_stabilizer)
		turret_dir = turn(turret_dir, deg)
